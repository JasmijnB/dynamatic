...
class VerilogEmitter(Emitter):
    """
    A context object to replace global variables for Verilog code generation.
    ...
    def bin_to_str(self, bin: Bin, meta: Meta) -> str:
        ...
        # Verilog has no concatenation operator, it uses the {a, b} form instead
    ...
    def logic_reg_init(self, logic: Logic, enable=None, init=None) -> None:
        """
        Generates a clocked snippet that sets up the register's behavior.
        For example,
    ...
    def mask_less(din, size) -> str:
        """
        Example:
            mask_less(3, 5)  # Output: "5'b00111"
            mask_less(2, 6)  # Output: "6'b000011"
            mask_less(5, 5)  # Output: "5'b11111"
            mask_less(0, 4)  # Output: "4'b0000"
        """
        if din > size:
