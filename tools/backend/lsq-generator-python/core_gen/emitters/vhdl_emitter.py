...
class VHDLEmitter(Emitter):
    ...
    def int_to_str(din: int, size=None, meta=None) -> str:
        ...
        if size == None:
            if din == 1:
                ...
                raise ValueError(
                    'int_to_str: Invalid value for size=None! Cannot represent the value as a single bit!'
                )
        else:
    ...
    def mask_less(din, size) -> str:
        """
        Example:
            mask_less(3, 5)  # Output: "00111"
            mask_less(2, 6)  # Output: "000011"
            mask_less(5, 5)  # Output: "11111"
            mask_less(0, 4)  # Output: "0000"
        """
        if din > size:
