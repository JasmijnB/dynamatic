...
// Testbench for structure_cross_group_matrix
// Same DUT shape as structure_cross_group, but the cross-BB dependency
// checker uses the 2D pq x sq dependency-matrix variant (UseDependencyMatrix)
// instead of the access-disparity state machine. The test sequence is
// identical to structure_cross_group_tb.sv so the two can be compared
// directly: both must satisfy the exact same allow_sq_access contract.
...
module structure_cross_group_matrix_tb;
...
// DUT instantiation
// ===----------------------------------------------------------------------===
structure_cross_group_matrix dut (
    .clk (clk),
    .rst (rst),
...
initial begin
    $dumpfile("structure_cross_group_matrix_tb.vcd");
    $dumpvars(0, structure_cross_group_matrix_tb);
    ...
    // ------------------------------------------------------------------
    // TEST 7: PQ occupancy back-pressure — BB0's matrix-tracked PQ
    // occupancy fills up.
    // dep_depth = pq.num_entries = 8 (matrix variant tracks occupancy 1:1
    // with the PQ's own physical entries, no dep_entry_ratio scaling).
    ...
    begin
        ...
        ticks(3);
        check(bb_ready_0, 0, "T7: PQ occupancy full — back-pressure asserted");
        // Service the head load: completing it advances pq_done_en at the PQ
        // head slot, clearing that slot's occupancy bit and releasing
        // back-pressure.
        fork
        ...
        join
        ticks(3);
        check(bb_ready_0, 1, "T7: PQ occupancy has room again after load completed");
    end
    ...
    // second successor's dependency set must extend to cover that new
    // predecessor entry. For the matrix variant, this just means S3's
    // dep_matrix row (snapshotted from PQ occupancy at S3's own BB
    // execution) must include PQ4, since P4 executed before S3 did —
    // unlike the access-disparity state machine, the matrix has no
    // "inherit the previous successor's boundary" failure mode (each
    // successor's row is stamped fresh at its own allocation), but this
    // scenario is still worth comparing directly between both variants.
    //
    // Mirrors the exact scenario found in the `lu` integration-test failure
    // (lsq7's dp_q3_q2) that motivated structure_cross_group_tb.sv's own
    // T10. Execution order: P0 S0 P1 S1 P2 P3 S2 P4 S3. S2 is allocated
    // right after P2/P3 push, so its row only depends on PQ0..PQ3 — correct,
    // since by execution order S2 only needs to wait through P3. S3 is
    // allocated after P4 has ALSO pushed, so its row must additionally
    // depend on PQ4 — S3 must not be released until PQ4 (P4's load) retires,
    // even after PQ0..PQ3 have all already retired.
    // ------------------------------------------------------------------
    test_counter = 10;
    reset();
    begin
        ...
        // S2's BB execution allocates its dep_matrix row from the PQ
        // occupancy snapshot at that point: {PQ0, PQ1, PQ2, PQ3}.
        bb1_execute();  st_send_addr(A2);

        // P4 pushes (sets PQ4 occupied), then S3's BB execution allocates
        // its row from the now-updated PQ occupancy snapshot: {PQ0, PQ1,
        // PQ2, PQ3, PQ4} — correctly including PQ4 since P4 executed before
        // S3 did.
        ...
        // A4's load is deliberately left pending: store 4 (S3, whose
        // dep_matrix row includes PQ4) must NOT be released until A4's own
        // load completes, even though PQ0..PQ3 (also set in S3's row) have
        // all already retired.
        fork
